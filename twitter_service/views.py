...
class StdOutListener(StreamListener):
                ...
                def on_data(self, data):
                    ...
                    try:
                        ...
                        pass
                    try:
                        curr.execute("""INSERT INTO TwitterFeed2 (UserID, Date, X, Y, Text, location) VALUES
                                (%s, %s, %s, %s, %s, %s);""",
                                (decoded['id_str'],decoded['created_at'],XY[0],XY[1],decoded['text'].encode('ascii', 'ignore'),decoded['user']['location'].encode('ascii', 'ignore')))
                        db.commit()
                    except Exception,e:
                        print e
                        pass
                ...
                def on_status(self, status):
                    ...
                    curr.execute("""INSERT INTO TwitterFeed2 (UserID, Date, X, Y, Text, location) VALUES
                        (%s, %s, %s, %s, %s, %s);""",
                        (status.id_str,status.created_at,XY[0],XY[1],text,location))
                    db.commit()
